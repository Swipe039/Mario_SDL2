...
SDL_Texture* loadTexture(const char* path, SDL_Renderer* renderer) {
    ...
    if (!surface) {
        std::cout << "Failed to load " << path << "! IMG_Error: " << IMG_GetError() << std::endl;
        return nullptr;
    }
    SDL_Texture* texture = SDL_CreateTextureFromSurface(renderer, surface);
    SDL_FreeSurface(surface);
    if (!texture) {
        std::cout << "Failed to create texture from " << path << "! SDL_Error: " << SDL_GetError() << std::endl;
    }
    return texture;
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (!jumpLeftTextures[i] || !jumpRightTextures[i]) {
            // Giải phóng tài nguyên và thoát nếu có lỗi
            for (int j = 0; j < i; j++) {
    ...
    while (running) {
        ...
        switch (state) {
            ...
            case GAMEPLAY: {
                ...
                updateEntity(mario, enemies, enemiesAlive, running, dropThrough, cameraX, state, stars, tilemaps[currentMap]);
